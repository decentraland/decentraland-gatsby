...
export function AuthChainStandar() {
  ...
  const [signing, sign] = useAsyncTask(async () => {
    ...
    if (identity && url) {
      ...
      // Must match what verifiers rebuild: method and path lowercased, metadata joined
      // verbatim. This previously folded the whole payload and the metadata on top of that,
      // so headers generated here were rejected by any @dcl/crypto-middleware 6 verifier
      // whenever the metadata contained an uppercase character.
      const chain = await signPayload(
        ...
        signedFetchPayload(
          ...
          url.pathname,
          String(timestamp),
          (state.metadata && JSON.stringify(state.metadata)) || '{}'
        )
      )
...
export function AuthChainRaw() {
  ...
  const [signing, sign] = useAsyncTask(
    async (payload) => {
      const identity = getCurrentIdentity()
      if (identity) {
        // Signed exactly as typed. This helper is the raw escape hatch, so folding the input would
        // make it impossible to reproduce a payload whose casing matters — which, since 6.x binds
        // the metadata bytes, is now every payload carrying metadata.
        const chain = await signPayload(getCurrentIdentity(), payload)
        setChain(chain)
      }
    },
    [chain]
