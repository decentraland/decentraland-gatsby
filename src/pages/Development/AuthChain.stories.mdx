...
export function AuthChainBulk({ chain, timestamp, metadata, options }) {
  let bulk = []
  if (chain) {
    bulk.push(
      ...chain.map(
...
<Grid.Row>
<Grid.Column mobile="16">
<Source language="bash" code={bulk.join('\n')} />
</Grid.Column>
</Grid.Row>
)
}
...
<Tabs.Tab active={activeTab === TabOptions.BULK} onClick={() => setActiveTab(TabOptions.BULK)}>BULK</Tabs.Tab>
</Tabs>
        ...
        {activeTab === TabOptions.BULK && <AuthChainBulk
          chain={chain}
          timestamp={timestamp}
          metadata={metadata}
        />}
