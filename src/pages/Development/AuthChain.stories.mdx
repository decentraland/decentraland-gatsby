...
export function CopyBulk({ chain, timestamp, metadata }) {
  let bulk = []
  if (chain) {
    bulk.push(
      ...chain.map(
...
return navigator.clipboard.writeText(bulk.join('\n'))
}
...
const copyAllHandler = useCallback(() => {
CopyBulk({ chain, timestamp, metadata })
setCopyAllText("Copied!")
setTimeout(() => {
setCopyAllText("Copy All")
}, 1000)
}, [chain, timestamp, metadata, setCopyAllText])
