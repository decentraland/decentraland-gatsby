...
export default React.memo(function WalletSelector () {
  const [ availableProviders, setAvailableProviders ] = useState<(readonly [ProviderType, LoginModalOptionType])[]>([])
  ...
  return <LoginModal open={!auth && state.selecting} loading={availableProviders.length === 0 || state.loading} onClose={() => state.select(false)}>
      {availableProviders.map(([providerType, optionType]) => {
        switch (providerType) {
          case ProviderType.INJECTED:
            return <LoginModal.Option key={providerType} type={optionType} onClick={() => state.connect(providerType, chainId)} />
          case ProviderType.FORTMATIC:
            return <LoginModal.Option key={providerType} type={optionType} onClick={() => state.connect(providerType, chainId)} />
          default:
            return null
