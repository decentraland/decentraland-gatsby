...
> The *sign hook* allow to sign message usign a pair address and provider
...
<ArgsTable rows={[
  ...
  Args.returns('transactions', 'list of transactions', `null | Transaction[]`),
  Args.returns('state.add', 'add a transaction using it hash and a payload', `(tx: string, payload: Record<string, any>) => void`),
  Args.returns('state.get', 'get the latest transaction usgin the id property in the payload', `(id: string) => Transaction | null`),
...
export const SendTransaction = () => {
  ...
  const [ transactions, transactionState ] = useTransaction(account, accountState.chainId)
  const [ sending, setSending ] = useState(false)
  useEffect(() => {
    if (sending) {
      Promise.resolve()
        .then(() => new Eth(accountState.provider)
          .sendTransaction({
            from: account,
            to: account,
            value: 1
    ...
    <Grid>
      {!account && <Grid.Row>
        <Grid.Column tablet="16">
          <Blockquote>
            <Paragraph small secondary>This feature requires to login, try to connect using the <Link href="/?path=/story/hooks-useauth--page#connect-preview">`useAuth`</Link> hook.</Paragraph>
          </Blockquote>
        </Grid.Column>
      </Grid.Row>}
      <Grid.Row>
        <Grid.Column tablet="16">
          <Button primary size="small" style={{ minWidth: '200px', margin: '0 1em 0 0' }} disabled={!account} loading={sending} onClick={() => setSending(true)}>Send transaction</Button>
          <Button primary size="small" style={{ minWidth: '200px', margin: '0 1em 0 0' }} disabled={!account || transactions.length === 0} loading={sending} onClick={() => transactionState.clear()}>Clear</Button>
          <Source language="ts" code={JSON.stringify(transactions, null, 2)} />
        </Grid.Column>
      </Grid.Row>
    </Grid>
  </Container>
