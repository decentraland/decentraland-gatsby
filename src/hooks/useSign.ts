...
export default function useSign(address?: string | null, provider?: Provider | null) {
  const [ state, setState ] = useState<SignState>({ message: null, signature: null, signing: false })

  useEffect(() => {
    if (state.signing && address && provider) {
      new Personal(provider)
        .sign(state.message || '', Address.fromString(address), '')
        .then((signature) => setState({ message: state.message, signature, signing: false }))
        .catch(() => setState({ message: null, signature: null, signing: false }))
    }
  ...
  return [ { signature: state.signature, message: state.message }, { sign, signing: state.signing } ]
}