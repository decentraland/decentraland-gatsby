...
<ArgsTable rows={[
  ...
  Args.returns('sign.signature', 'signature of `sign.message`', `string | null`),
  Args.returns('sign.message', 'message signed at `sign.signature` ', `string | null`),
  Args.returns('state.signing', '`true` when is waiting for a user initeraction', `boolean`),
  Args.returns('state.sign', 'asks user to sign a message', `(mesage: string) => void`)
]} />
    ...
    <Grid>
      ...
      <Grid.Row>
        <Grid.Column tablet="8">
          <Textarea value={message} minHeight={170} onChange={(e, { value }) => setState(value || '') } />
        </Grid.Column>
        <Grid.Column tablet="8">
          <Button primary size="small" style={{ minWidth: '200px', margin: '0 auto' }} disabled={!account} loading={signatureState.signing} onClick={() => signatureState.sign(message)}>Sign</Button>
...
export function Sign () {
  ...
  return <div>
    ...
    <button disabled={!account} onClick={() => signatureState.sign(message)}>sign</button>
