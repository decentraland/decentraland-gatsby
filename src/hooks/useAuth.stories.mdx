...
export const SwitchExample = () => {
  ...
  return <Container>
    <div style={{ display: 'flex', flexWrap: 'wrap' }}>
      {chains.map(chainId => {
        return <Button
          ...
          style={{ margin: '0 .5em .5em 0' }}
          loading={state.loading}
          disabled={!account || state.chainId === chainId || state.providerType !== ProviderType.INJECTED}
    ...
    {state.providerType !== ProviderType.INJECTED && <div style={{ paddingTop: '1em' }}>
      <Paragraph primary small>Network switch is only available for INJECTED providers</Paragraph>
    </div>}
  </Container>
}
