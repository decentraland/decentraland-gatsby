...
export const CatalystLoader = React.memo((props) => {
  const [comms, commsState] = useAsyncState(
    () => Catalyst.from(props.baseUrl).getCommsStatus(),
    [props.baseUrl]
  )
  const [lambdas, lambdasState] = useAsyncState(
    () => Catalyst.from(props.baseUrl).getLambdasStatus(),
    [props.baseUrl]
  )
  const [content, contentState] = useAsyncState(
    () => Catalyst.from(props.baseUrl).getContentStatus(),
    [props.baseUrl]
  )
  const [profile, profileState] = useAsyncState(
    () => Catalyst.from(props.baseUrl).getProfile(props.user),
    [props.user],
      ...
      <Card.Content>
        <div style={{ display: 'flex', alignItems: 'top' }}>
          ...
          >
            ...
            {comms && (
              <div style={{ paddingBottom: '16px' }}>
                <Stats title="Users">{comms.usersCount || '0'}</Stats>
              </div>
            )}
          </div>
          <div
