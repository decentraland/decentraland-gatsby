...
describe('./src/entities/Prometheus/cluster', () => {
  describe('merge', () => {
    test(
      `shoudl return a new instance`,
      mockClusterRegistry(async (mock) => {
  ...
  describe(`metrics`, () => {
    test(
      `on master should return the metrics`,
      mockClusterRegistry(async (mock) => {
        mock.isMaster.mockReturnValue(true)
        mock.addEventListener.mockReturnValue()
    ...
    test(
      `on fork should send a requet message to the master instance`,
      mockClusterRegistry(async (mock) => {
        mock.isMaster.mockReturnValue(false)
        mock.addEventListener.mockReturnValue()
